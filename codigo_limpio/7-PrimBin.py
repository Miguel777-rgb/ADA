...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("analisis_prim_graphistry.log", mode='w', encoding='utf-8'), logging.StreamHandler()]
)
...
def encontrar_nodo_inicio_valido(graph: ig.Graph, nodes_in_component: List[int]) -> Optional[int]:
...
def prim_mst_forest_manual(graph: ig.Graph) -> Tuple[List[Tuple[int, int]], float]:
    logging.info("Calculando el Bosque de Expansión Mínima con Prim manual...")
    start_time = time.time()
    ...
    logging.info(f"Grafo tiene {len(components)} componentes. Calculando un MST para cada uno.")
    for component_nodes in tqdm(components, desc="Procesando Componentes"):
        if len(component_nodes) <= 1: continue
        start_node = encontrar_nodo_inicio_valido(graph, component_nodes)
        if start_node is None: continue
        key = {node: float('inf') for node in component_nodes}
        key[start_node] = 0
        pq = MinHeap()
        for node in component_nodes: pq.push(key[node], node)
        while not pq.is_empty():
            ...
            for neighbor_v in graph.neighbors(u, mode='all'):
                if neighbor_v in key: 
                    edge = graph.es[graph.get_eid(u, neighbor_v, directed=False)]
                    edge_weight = edge['weight']
                    if not in_mst[neighbor_v] and edge_weight < key[neighbor_v]:
                        key[neighbor_v] = edge_weight
                        parent[neighbor_v] = u
                        pq.update(neighbor_v, edge_weight)
    logging.info(f"Algoritmo de Prim completado en {time.time() - start_time:.4f} s.")
...
def visualizar_mst_con_graphistry(graph: ig.Graph, mst_edges: List[Tuple[int, int]]):
    ...
    if len(mst_edges) > MAX_ARISTAS_MST_PARA_GRAPHISTRY:
        logging.warning(f"El MST ({len(mst_edges)} aristas) supera el límite de {MAX_ARISTAS_MST_PARA_GRAPHISTRY}. Tomando muestra.")
    ...
    logging.info(f"La muestra del MST contiene {len(nodos_unicos)} nodos únicos.")
    ...
    # Usar .replace() en lugar de .map_dict()
    ...
    try:
        ...
        logging.info(f"Visualización del MST creada. Ábrela en tu navegador: {url}")
        print(f"\n--> URL de la visualización del MST: {url}\n")
        webbrowser.open(url)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if mst_aristas:
        print("\n" + "="*60)
        print("  VISUALIZACIÓN DEL MST DEL SUBGRAFO CON GRAPHISTRY")
        print("="*60)
        ...
        logging.error("No se pudo calcular el MST para el subgrafo.")